...
diagonal.forEach(function(coords) {
    let x1 = parseInt(coords[0].split(',')[0]);
    let y1 = parseInt(coords[0].split(',')[1]);
    let x2 = parseInt(coords[1].split(',')[0]);
    let y2 = parseInt(coords[1].split(',')[1]);
    let xBounds = ([x1, x2]).sort(function(a, b) {
        return a - b;
    });
    let yBounds = ([y1, y2]).sort(function(a, b) {
        return a - b;
    });

    if ((x1 < x2 && y1 < y2) || (x1 > x2 && y1 > y2)) {
        // Line is going left to right top to bottom
        for (let i = xBounds[0]; i <= xBounds[1]; i++) {
            for (let j = yBounds[0]; j <= yBounds[1]; j++) {
                if ((j - i) == (yBounds[0] - xBounds[0])) {
                    if (map[j][i] == '.') {
                        map[j][i] = '1';
                    } else {
                        map[j][i] = (parseInt(map[j][i]) + 1).toString();
                    }
                }
            }
        }
    } else {
        // Line is going left to right bottom to top
        for (let i = xBounds[0]; i <= xBounds[1]; i++) {
            for (let j = yBounds[1]; j >= yBounds[0]; j--) {
                if ((i + j) == (xBounds[0] + yBounds[1])) {
                    if (map[j][i] == '.') {
                        map[j][i] = '1';
                    } else {
                        map[j][i] = (parseInt(map[j][i]) + 1).toString();
